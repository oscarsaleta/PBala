...
int main(int argc, char *argv[]) {
	...
	while (1) {
		...
		// Generate execution
		/*sprintf(command,"%s %d %s > %s/%d_out.txt 2> %s/%d_err.txt",
			inp_programFile,taskNumber,arguments,out_dir,taskNumber,out_dir,taskNumber);*/
		sprintf(command,"%s %d %s/%d_out.txt %s",inp_programFile,taskNumber,out_dir,taskNumber,arguments);
		system(command);
